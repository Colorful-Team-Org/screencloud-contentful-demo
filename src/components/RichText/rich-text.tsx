...
export const RichText: React.FC<RichTextProps> = (props: RichTextProps) => {
  // const findAsset = (id: string) => props.assets?.find(asset => asset?.sys.id === id);

  return (
    <div className={`${styles.richtext} ${props.className}`} style={props.style}>
      {documentToReactComponents(props.document, {
        renderNode: {
          [BLOCKS.EMBEDDED_ASSET]: node => {
            ...
            return (
              /* <ImageAsset {...asset} /> */
              <EmbeddedAssetBlock id={node.data.target.sys.id} preview={props.preview} />
            );
          },
        },
